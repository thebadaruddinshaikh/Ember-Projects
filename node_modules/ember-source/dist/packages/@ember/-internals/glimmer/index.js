...
import { get, PROPERTY_DID_CHANGE, set, computed, alias, tracked, tagForObject, objectAt, tagForProperty, _getProp, _setProp } from '@ember/-internals/metal';
import { assert, deprecate, debugFreeze, runInDebug, warn } from '@ember/debug';
import { EMBER_COMPONENT_IS_VISIBLE, JQUERY_INTEGRATION, SEND_ACTION, PARTIALS, COMPONENT_MANAGER_STRING_LOOKUP } from '@ember/deprecated-features';
...
import { logTrackingStack, beginUntrackFrame, endUntrackFrame, valueForTag, beginTrackFrame, consumeTag, endTrackFrame, validateTag, createTag, dirtyTag, untrack, deprecateMutationsInTrackingTransaction, CONSTANT_TAG, isTracking, tagFor, setTrackingTransactionEnv, createUpdatableTag, CURRENT_TAG, createCache, getValue } from '@glimmer/validator';
import { clearElementView, clearViewElement, getViewElement, MUTABLE_CELL, addChildView, setElementView, setViewElement, ActionSupport, ChildViewsSupport, ClassNamesSupport, CoreView, ViewMixin, ViewStateSupport, TextSupport, isSimpleClick, jQuery, jQueryDisabled, constructStyleDeprecationMessage, ActionManager, getViewId } from '@ember/-internals/views';
import { registerDestructor, associateDestroyableChild, destroy } from '@glimmer/destroyable';
import { setInternalHelperManager, setInternalComponentManager, setComponentTemplate, setInternalModifierManager, getInternalHelperManager, helperCapabilities, setHelperManager, capabilityFlagsFrom, getComponentTemplate, getInternalComponentManager, componentCapabilities, modifierCapabilities, setComponentManager } from '@glimmer/manager';
...
function makeClosureAction(context, target, action$$1, processArgs, debugKey) {
  ...
  if (typeof action$$1[INVOKE] === 'function') {
    deprecate(`Usage of the private INVOKE API to make an object callable via action or fn is no longer supported. Please update to pass in a callback function instead. Received: ${String(action$$1)}`, false, {
      until: '3.25.0',
...

{
  ...
  Object.defineProperty(Component, 'reopenClass', {
    ...
    enumerable: false,
    writable: true,
    value: function reopenClass(...args) {
      if (this === Component) {
        deprecate('Reopening the Ember.Component super class itself is deprecated. ' + 'Consider alternatives such as installing event listeners on ' + 'the document or add the customizations to specific subclasses.', false, {
          id: 'ember.component.reopen',
          for: 'ember-source',
...

{
  ...
  Object.defineProperty(Checkbox, 'reopen', {
    ...
    value: function reopen(...args) {
      if (this === Checkbox) {
        deprecate('Reopening Ember.Checkbox is deprecated. Consider implementing your own ' + 'wrapper component or create a custom subclass.', false, {
...

{
  ...
  Object.defineProperty(TextField, 'reopen', {
    ...
    value: function reopen(...args) {
      if (this === TextField) {
        deprecate('Reopening Ember.TextField is deprecated. Consider implementing your own ' + 'wrapper component or create a custom subclass.', false, {
          id: 'ember.built-in-components.reopen',
          for: 'ember-source',
          since: {
            enabled: '3.27.0'
...

{
  ...
  Object.defineProperty(TextArea, 'reopen', {
    ...
    value: function reopen(...args) {
      if (this === TextArea) {
        deprecate('Reopening Ember.TextArea is deprecated. Consider implementing your own ' + 'wrapper component or create a custom subclass.', false, {
          id: 'ember.built-in-components.reopen',
          for: 'ember-source',
          since: {
            enabled: '3.27.0'
  ...
  Object.defineProperty(LinkComponent, 'reopen', {
    ...
    value: function reopen(...args) {
      if (this === LinkComponent) {
        deprecate('Reopening Ember.LinkComponent is deprecated. Consider implementing your own ' + 'wrapper component or create a custom subclass.', false, {
          id: 'ember.built-in-components.reopen',
          for: 'ember-source',
          since: {
            enabled: '3.27.0'
...
function handleDeprecatedAttributeArguments(target, bindings) {
  {
    ...
    bindings.forEach(binding => {
      ...
      let superGetter = () => undefined;

      if (superDescriptor) {
        assert(`[BUG] expecting ${attribute} to be a getter on <${angle}>`, typeof superDescriptor.get === 'function');
      ...
      Object.defineProperty(prototype, attribute, {
        ...
        get() {
          if (argument in this.args.named) {
            deprecate(`Passing the \`@${argument}\` argument to <${angle}> is deprecated. ` + `Instead, please pass the attribute directly, i.e. \`<${angle} ${attribute}={{...}} />\` ` + `instead of \`<${angle} @${argument}={{...}} />\` or \`{{${curly} ${argument}=...}}\`.`, false, {
...
let handleDeprecatedEventArguments = NOOP$2;

{
  const EVENTS = new WeakMap();
  const EMPTY_EVENTS = Object.freeze({});

  const getEventsMap = owner => {
    let events = EVENTS.get(owner);

    if (events === undefined) {
      events = EMPTY_EVENTS;
      let eventDispatcher = owner.lookup('event_dispatcher:main');
      ...
      if (eventDispatcher !== null && eventDispatcher !== undefined) {
        ...
        if ('_finalEvents' in eventDispatcher && eventDispatcher._finalEvents !== null && eventDispatcher._finalEvents !== undefined) {
          assert('[BUG] invalid _finalEvents on event dispatcher', typeof eventDispatcher._finalEvents === 'object');
  ...
  handleDeprecatedEventArguments = (target, extraEvents = []) => {
    ...
    Object.defineProperty(prototype, 'isSupportedArgument', {
      ...
      enumerable: false,
      value: function isSupportedArgument(name) {
        let events = [...ObjectValues(getEventsMap(this.owner)), 'focus-in', 'focus-out', 'key-press', 'key-up', 'key-down'];
    ...
    class DeprecatedEventHandlersModifier extends InternalModifier {
      ...
      install() {
        ...
        let entries = [...ObjectEntries(getEventsMap(this.owner)), ...extraEvents];
      ...
      get component() {
        ...
        assert(`[BUG] must pass <${angle}> component as first argument`, component instanceof target);
        return component;
      }
      ...
      listenerFor(event, argument) {
        ...
        if (argument in this.args.named) {
          deprecate(`Passing the \`@${argument}\` argument to <${angle}> is deprecated. ` + `This would have overwritten the internal \`${argument}\` method on ` + `the <${angle}> component and prevented it from functioning properly. ` + `Instead, please use the {{on}} modifier, i.e. \`<${angle} {{on "${event}" ...}} />\` ` + `instead of \`<${angle} @${argument}={{...}} />\` or \`{{${curly} ${argument}=...}}\`.`, !(argument in this), {
            id: 'ember.built-in-components.legacy-attribute-arguments',
            for: 'ember-source',
            since: {
              enabled: '3.27.0'
          ...
          deprecate(`Passing the \`@${argument}\` argument to <${angle}> is deprecated. ` + `Instead, please use the {{on}} modifier, i.e. \`<${angle} {{on "${event}" ...}} />\` ` + `instead of \`<${angle} @${argument}={{...}} />\` or \`{{${curly} ${argument}=...}}\`.`, argument in this, {
            id: 'ember.built-in-components.legacy-attribute-arguments',
            for: 'ember-source',
            since: {
              enabled: '3.27.0'
    ...
    setInternalModifierManager(new InternalModifierManager(DeprecatedEventHandlersModifier, 'deprecated-event-handlers'), DeprecatedEventHandlersModifier);
    ...
    Object.defineProperty(prototype, 'listenerFor', {
      ...
      value: function listenerFor(name) {
        ...
        if (jQuery && !jQueryDisabled) {
          return event => listener(new jQuery.Event(event));
        } else {
          return listener;
        }
      }
...
class AbstractInput extends InternalComponent {
  ...
  shouldModernize() {
    ...
    ) && Component._wasReopened === false && TextSupport._wasReopened === false && TargetActionSupport._wasReopened === false;
  }
    ...
    Object.defineProperty(prototype, 'listenerFor', {
      ...
      value: function listenerFor(name) {
        ...
        if (typeof actionName === 'string') {
          deprecate(`Passing actions to components as strings (like \`<${angle} @${name}="${actionName}" />\`) is deprecated. ` + `Please use closure actions instead (\`<${angle} @${name}={{action "${actionName}"}} />\`).`, false, {
          ...
          if (isView(caller)) {
            listener = (...args) => caller.send(actionName, ...args);
          } else {
            assert(`The action '${actionName}' did not exist on ${caller}`, typeof caller[actionName] === 'function');
            listener = caller[actionName];
          }

          let deprecatedListener = (...args) => {
            deprecate(`Passing actions to components as strings (like \`<${angle} @${name}="${actionName}" />\`) is deprecated. ` + `Please use closure actions instead (\`<${angle} @${name}={{action "${actionName}"}} />\`).`, false, {
              id: 'ember-component.send-action',
              for: 'ember-source',
              since: {},
...
{
  handleDeprecatedFeatures(Input, [// Component
  'id', ['id', 'elementId'], 'class', ['class', 'classNames'], ['role', 'ariaRole'], // TextSupport
  'autocapitalize', 'autocorrect', 'autofocus', 'disabled', 'form', 'maxlength', 'minlength', 'placeholder', 'readonly', 'required', 'selectionDirection', 'spellcheck', 'tabindex', 'title', // TextField
  'accept', 'autocomplete', 'autosave', 'dir', 'formaction', 'formenctype', 'formmethod', 'formnovalidate', 'formtarget', 'height', 'inputmode', 'lang', 'list', 'max', 'min', 'multiple', 'name', 'pattern', 'size', 'step', 'width', // Checkbox
  'indeterminate']);
}
...
__decorate$2([action], Textarea.prototype, "change", null);

__decorate$2([action], Textarea.prototype, "input", null); // Deprecated features


{
  handleDeprecatedFeatures(Textarea, [// Component
  'id', ['id', 'elementId'], 'class', ['class', 'classNames'], ['role', 'ariaRole'], // TextSupport
  'autocapitalize', 'autocorrect', 'autofocus', 'disabled', 'form', 'maxlength', 'minlength', 'placeholder', 'readonly', 'required', 'selectionDirection', 'spellcheck', 'tabindex', 'title', // TextField
...
var queryParams = internalHelper(({
  ...
  return createComputeRef(() => {
    assert("The `query-params` helper only accepts named arguments, e.g. (query-params queryParamPropertyName='foo') as opposed to (query-params 'foo')", positional.length === 0);
    deprecate('The `query-params` helper is deprecated. Invoke `<LinkTo>` with the `@query` named argument and the `hash` helper instead.', false, {
      id: 'ember-glimmer.link-to.positional-arguments',
      until: '4.0.0',
      for: 'ember-source',
      url: 'https://deprecations.emberjs.com/v3.x#toc_ember-glimmer-link-to-positional-arguments',
...
const EMPTY_ARRAY$1 = [];
const EMPTY_QUERY_PARAMS$1 = {};
debugFreeze(EMPTY_ARRAY$1);
debugFreeze(EMPTY_QUERY_PARAMS$1);
...
class LinkTo extends InternalComponent {
  ...
  validateArguments() {
    assert('You attempted to use the <LinkTo> component within a routeless engine, this is not supported. ' + 'If you are using the ember-engines addon, use the <LinkToExternal> component instead. ' + 'See https://ember-engines.com/docs/links for more info.', !this.modernized || !this.isEngine || this.engineMountPoint !== undefined);
    assert('You must provide at least one of the `@route`, `@model`, `@models` or `@query` argument to `<LinkTo>`.', !this.modernized || 'route' in this.args.named || 'model' in this.args.named || 'models' in this.args.named || 'query' in this.args.named);
  ...
  shouldModernize() {
    return Boolean(true
    /* EMBER_MODERNIZED_BUILT_IN_COMPONENTS */
    ) && Component._wasReopened === false && TargetActionSupport._wasReopened === false && LinkComponent._wasReopened === false;
  }
...

{
  ...
  handleDeprecatedAttributeArguments(LinkTo, [// Component
  'id', ['id', 'elementId'], 'class', ['class', 'classNames'], ['role', 'ariaRole'], // LinkTo
...

  {
    ...
    Object.defineProperty(prototype, 'isSupportedArgument', {
      ...
      value: function isSupportedArgument(name) {
        if (this.modernized) {
          if (name === 'bubbles') {
            deprecate('Passing the `@bubbles` argument to <LinkTo> is deprecated. ' + 'Use the {{on}} modifier to attach a custom event handler to ' + 'control event propagation.', false, {
              id: 'ember.built-in-components.legacy-arguments',
              for: 'ember-source',
              since: {
                enabled: '3.27.0'
          ...
          if (name === 'preventDefault') {
            deprecate('Passing the `@preventDefault` argument to <LinkTo> is deprecated. ' + '`preventDefault()` is called automatically on events that are ' + 'handled by the <LinkTo> component to prevent the browser from ' + 'navigating away from the page.', false, {
              id: 'ember.built-in-components.legacy-arguments',
              for: 'ember-source',
              since: {
                enabled: '3.27.0'
      ...
      value: function preventDefault(event) {
        ...
        if ('preventDefault' in this.args.named) {
          ...
          if (isMissing(value) || value) {
            ...
            deprecate('Passing the `@preventDefault` argument to <LinkTo> is deprecated. ' + '`preventDefault()` should always be called on events that are ' + 'handled by the <LinkTo> component to prevent the browser from ' + 'navigating away from the page.', false, {
              id: 'ember.built-in-components.legacy-arguments',
              for: 'ember-source',
              since: {
                enabled: '3.27.0'
        ...
        if ('bubbles' in this.args.named) {
          ...
          if (value === false) {
            ...
            deprecate('Passing the `@bubbles` argument to <LinkTo> is deprecated. ' + '`stopPropagation()` is not automatically called so there is ' + 'no need to pass this argument when you DO want the event to ' + 'propagate normally', false, {
              id: 'ember.built-in-components.legacy-arguments',
              for: 'ember-source',
              since: {
                enabled: '3.27.0'
...

  {
    ...
    assert(`[BUG] expecting isDisabled to be a getter on <LinkTo>`, superDescriptor && typeof superDescriptor.get === 'function');
    let superGetter = superDescriptor.get;
...
if (DEBUG) {
  componentCapabilities$1 = (version, options) => {
    deprecate('Versions of component manager capabilities prior to 3.13 have been deprecated. You must update to the 3.13 capabilities.', version === '3.13', {
    ...
    return componentCapabilities(version, options);
  };

  modifierCapabilities$1 = (version, options) => {
    deprecate('Versions of modifier manager capabilities prior to 3.22 have been deprecated. You must update to the 3.22 capabilities.', version === '3.22', {
    ...
    return modifierCapabilities(version, options);
  };
}
...
export { RootTemplate, Checkbox, TextField, TextArea, LinkComponent, Input$1 as Input, Textarea$1 as Textarea, Component, Helper, helper, SafeString, escapeExpression, htmlSafe, isHTMLSafe$1 as isHTMLSafe, Renderer, _resetRenderers, renderSettled, getTemplate, setTemplate, hasTemplate, getTemplates, setTemplates, setupEngineRegistry, setupApplicationRegistry, INVOKE, OutletView, componentCapabilities$1 as componentCapabilities, modifierCapabilities$1 as modifierCapabilities, setComponentManager$1 as setComponentManager };
